...
async function saveProduct() {
    const editId = document.getElementById('edit_id') ? document.getElementById('edit_id').value : null;
    const fileInput = document.getElementById('foto_file');
    const nama = document.getElementById('nama').value;
    const harga = document.getElementById('harga').value;

    if (!nama || !harga) {
        return Swal.fire({ icon: 'warning', title: 'Data Belum Lengkap', text: 'Nama dan Harga wajib diisi!', background: '#111827', color: '#fff' });
    }
    ...
    if (fileInput.files.length > 0) {
        ...
        if (file.size > 2 * 1024 * 1024) {
            return Swal.fire({ icon: 'error', title: 'File Terlalu Besar', text: 'Maksimal ukuran gambar adalah 2MB', background: '#111827', color: '#fff' });
        }

        Swal.fire({ title: 'Sedang Mengunggah Gambar...', background: '#111827', color: '#fff', allowOutsideClick: false, didOpen: () => Swal.showLoading() });
        imgUrl = await uploadImage(file);
        
        if (!imgUrl) {
            return Swal.fire({ icon: 'error', title: 'Upload Gagal', text: 'Gagal mengunggah gambar ke storage', background: '#111827', color: '#fff' });
        }
    }
    ...
    if(res.ok) {
        Swal.fire({ icon: 'success', title: editId ? 'Data Diperbarui' : 'Produk Berhasil Diupload', background: '#111827', color: '#fff' });
        ...
        Swal.fire({ icon: 'error', title: 'Gagal Menyimpan', text: 'Terjadi kesalahan saat menyimpan ke database', background: '#111827', color: '#fff' });
    }
}
    ...
    data.forEach(item => {
        table.innerHTML += `
            <tr class="border-b border-gray-900 hover:bg-gray-900/50 transition">
                <td class="p-4 font-bold uppercase tracking-tight">
                    ${item.nama}
                ...
                <td class="p-4 text-center">
                    <button onclick='prepareEdit(${JSON.stringify(item)})' class="text-blue-400 text-xs font-bold uppercase mx-2 tracking-widest">Edit</button>
                    <button onclick='viewDetail(${JSON.stringify(item)})' class="text-gray-400 text-xs font-bold uppercase mx-2 tracking-widest">Detail</button>
...
async function deleteData(id) {
    const ask = await Swal.fire({ title: 'Hapus Produk?', text: `ID: ${id}`, background: '#111827', color: '#fff', showCancelButton: true, confirmButtonColor: '#d33' });
    if(ask.isConfirmed) {
